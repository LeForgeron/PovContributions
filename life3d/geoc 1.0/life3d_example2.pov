...
#declare Life3D_gbDiedTexture=texture{                
        ...
        pigment{White transmit 1} // invisible?
}


//#declare Life3D_diffScale=0;// .5 makes 'old'  cells 50% larger than new ones.
...
#declare Life3D_livePercent=8; // livePercent=10 makes 10 percent of the array live cells                                           
...
//      could use Life3D_tempCellfilename to pickup from the end
//      of the last run
...
#declare myrules=array[4]{5,7,4,4}
//#declare myrules=rules5766
// The first two numbers in the rules array tell how many live neighbors a live
// cell must have in order to survive to the next generation. 
// For example, if you use a rule array 
//        array[4]{5,7,6,6}
// a live cell must have from 5 through 7 live neighbors in order to survive.
// The second pair (6,6 in the example) tell how many neighbors a dead (empty)
// cell needs in order to come alive. 5,7,6,6 says an empty cell must have
// at least 6 live neighbors and no fewer than 6 live neighbors to come alive


// a declared 'Life3D_cellObject' replaces the default cube 
// cells are 1x1, and cellObjects should be centered on <0,0,0>
//#declare Life3D_cellObject=sphere{0,1/2}

// Life3D_Tiles=0 makes just one cell array
// Life3D_Tiles=1 makes a copy of the array in 26 directions from the original
//         extending the array by tiling
// Life3D_Tiles=2 extends the array by tiling twice! 
#declare Life3D_Tiles=1;
                            
// Life3D_leaveCorpses=1 will leave a 'corpse' cell for cells which were alive
// last generation but died.
#declare Life3D_leaveCorpses=0;    
       
#declare Life3D_blobStrength=2;
// the scene dimensions of the entire cell array is xsize x ySize x zsize
intersection{
        object{
            Life3D(xsize,ySize,zsize,myrules,Life3D_fromRandom,Life3D_blobCells,Life3D_toruswrap,frame_number-1) 
        }            
        box{<-xsize,-ySize,-zsize>,<xsize,ySize,zsize> pigment{Yellow }}
}
// ---- end of scene  
