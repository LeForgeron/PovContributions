 ...
 *       08-jun-2012  Frame order is rearranged to match the user manual;
 ...
 * 1.2   01-mar-2013  Demo frames added for elliptical toroids.
 *       21-apr-2013  Tweaked the sky, added clouds, and reworked the lighting;
 *                    brightened the textures.
 *       21-apr-2013  Added checks for negative sqrt() operands.
 * 1.3   15-jun-2013  Changed blob function names.
 */
...
#declare c_Ambience = rgb (Rad? 0: <0.186, 0.200, 0.242>);
...
{ pigment
  { gradient y color_map
    { [0 rgb <0.476, 0.571, 0.838>]
      [1 rgb <0.057, 0.095, 0.229>]
    }
  }
...
#declare Boosts = array[6] { 1, 1, 1.15, 1, 0.85, 1 }

#declare Choose = mod (Test, 6);
#declare t_Sample = texture
{ pigment { rgb Vary[Choose] }
  finish
  { specular 0.154 roughness 0.025
    diffuse 0.6 * Boosts[Choose] ambient c_Ambience * Boosts[Choose]
  }
}

#declare Choose = mod (Test + 4, 6);
#declare t_Special = texture
{ pigment { rgb Vary[Choose] }
  finish
  { reflection { 0.1 metallic }
    diffuse 0.54 * Boosts[Choose]
    ambient c_Ambience * 0.9 * Boosts[Choose]
    brilliance 1.86
...
{ pigment { rgb <0.61, 0.61, 0.64> }
  ...
  { reflection { 0.9 metallic }
    diffuse 0.06
    ambient c_Ambience * 0.1
    brilliance 1.86
    specular 942 metallic
    roughness 0.000123
  }
}
...
{ RE_Box (<-R1, H0, -R1>, <R1, H1, R1>, rPLAT, no)
  ...
  { pigment { Red_Marble scale 2 translate -0.5 }
    finish
    { specular 5.233 roughness 0.0025
...
#switch (Test)
  ...
  #case (26)
    ...
    { function
      ...
      contained_by
      { box { -<0, RIN, R + EDGE>, <RIN + R, EDGE - RIN, R + EDGE> }
      }
    }
    #declare Sample = union
  ...
  #case (28)
    ...
    { RE_Cylinder (R*y, (ROUT - 4) * y, ROUT, R, no)
      ...
      object { End_blob texture { t_Special } }
