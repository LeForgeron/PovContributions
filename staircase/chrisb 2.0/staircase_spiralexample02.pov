...
/**********************************************************************************/
/* staircase_spiralexample02.pov  -  Created by Chris Bartlett March 2008         */
/*                                                                                */
...
// Use simpler newel post and baluster shapes
#include "StairCase_Samples.inc"
#declare StairCase_NewelPost = StairCase_NewelShape ("Bevelled")
#declare StairCase_Baluster  = StairCase_BalusterShape ("Bevelled")

// Change the default textures so that we have a glass handrail and shiny metal fittings
...
#declare StairCase_NewelTexture = texture {
  pigment {rgb <1,1,0.5>}
  finish {reflection 0.5 phong 0.5}
}

#declare StairCase_StringerTexture = texture {
  pigment {rgb <1,1,0.5>}
  finish {reflection 0.5 phong 0.5}
}
...
#declare InternalWallStructure = union {
  ...
  #local I = 0;
  #while (I<BuildingWidth/2-0.5)
    object {Door pigment {Dark_Wood scale 0.5 translate 200*z*rand(MySeed)} rotate -y*90 translate <-FoyerWidth/2,0, I>} 
    object {Door pigment {Dark_Wood scale 0.5 translate 200*z*rand(MySeed)} rotate -y*90 translate <-FoyerWidth/2,0,-I>} 
    #local I = I + 4;
  #end
  #local I = 0;
  #while (I<FoyerWidth/2-2.5)
    object {Door pigment {Dark_Wood scale 0.5 translate 200*z*rand(MySeed)}translate < I,0,FoyerWidth/2>} 
    object {Door pigment {Dark_Wood scale 0.5 translate 200*z*rand(MySeed)}translate <-I,0,FoyerWidth/2>} 
    #local I = I + 4;
  #end
}
